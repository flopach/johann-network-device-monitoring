    ...
    def _restconf_get_request(self, yang_loc, inner_function):
        ...
        try:
            ...
            if r_get.status_code == 200:
                logger.debug(self.device_ip+" | 200 Response: Successfully received data from "+yang_loc)
                i = inner_function(r_get.json()[yang_loc])
                return 200, i

            elif r_get.status_code == 204:
                logger.warning(self.device_ip+" | 204 Response: Received no content from "+yang_loc)
            ...
            if err_code == 401:
                return 401, "Unauthorized! Check username or password."
            elif err_code == 400:
                return 400, "Bad Request! Could connect to the device, but bad request. Is RESTCONF enabled?"
            elif err_code == 404:
                return 404, "Bad Request! Could connect to the device, but bad request. Is RESTCONF enabled?"
            else:
            ...
            logger.exception(self.device_ip+" | Request Error: "+str(err))
            return 0, "Can't connect to the device! Please check IP address and logs."
