...
"""
Open csv from memory:
decoded_file = csv_file.read().decode()
file_from_string = StringIO(decoded_file)
csv_data = DictReader(file_from_string)


Open CSV:
if csv_filepath.lower().endswith((".csv")) != True:
            return 0,"Wrong Filetype! Please upload only CSV files."
        ...
        with open(csv_filepath, mode='r') as csv_file:
            ...
            imported_devices = 0
            no_devices = len(list(csv_reader))
            current_device = 1
            logger.info("CSV-File Import | Started for "+csv_file.name)
...
def send_dashboard(msg):
    channel_layer = get_channel_layer()
    async_to_sync(channel_layer.group_send)("statusgroup", {"type": "send_status", "msg": msg})
    print("rawjson")
